...
@section('content')
        ...
        <div class="modal-dialog modal-dialog-custom" role="document">
            <div class="modal-content">
                <div class="modal-header modal-header-custom">
                    <h3 class="modal-title modal-title-custom" id="exampleModalLongTitle">Plate Maintenance</h3>
                </div>
                <div class="modal-body">
                    <form>
                        ...
                        <div class="form-group row">
                            <div class="col-sm-6">
                                <div class="input-group">
                                    ...
                                    <select class="selectpicker form-control form-control-custom-modal" id="select_printer">
                                        <option></option>
                        ...
                        <div class="form-group row">
                            <div class="col-sm-6">
                                <div class="input-group">
                                    <span class="input-group-addon input-group-addon-custom-modal" id="sizing-addon2">Flexi-Die</span>
                                     <select class="selectpicker form-control form-control-custom-modal" id="select_mold_id_fd">
                                        <option></option>
                                        @foreach ($select_plate_id_fds as $select_plate_id_fd)
                                            <option>{{ $select_plate_id_fd->mold_id_fd }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="col-sm-6">
                                <div class="input-group">
                                    ...
                                    <select class="selectpicker form-control form-control-custom-modal" id="select_part_no" data-live-search="true">  
                                        {{--  <optgroup label="New">
                                            <option id="editable_part" class="editable" value="new">New Part No</option>
                                        </optgroup>  --}}
                                        <option></option>
                        ...
                        <div class="form-group row">
                            <div class="col-sm-6">
                                <div class="input-group">
                                    ...
                                    <select class="selectpicker form-control form-control-custom-modal" id="text_location">
                                        <option></option>
                    ...
                    <form>
                        <div class="form-group row">
                            <div class="col-sm-6">
                                <div class="input-group">
                                    <span class="input-group-addon input-group-addon-custom-modal" id="sizing-addon2">Printer</span>
                                    <select class="selectpicker form-control form-control-custom-modal" id="select_printer_modify">
                                        <option></option>
                                        @foreach ($select_printers as $select_printer)
                                            <option>{{ $select_printer->value }}</option>
                                        @endforeach
                        ...
                        <div class="form-group row">
                            <div class="col-sm-6">
                                <div class="input-group">
                                    <span class="input-group-addon input-group-addon-custom-modal" id="sizing-addon2">Flexi-Die</span>
                                    <select class="form-control form-control-custom-modal" id="select_mold_id_fd_modify">
                                        <option></option>
                                        @foreach ($select_plate_id_fds as $select_plate_id_fd)
                                            <option>{{ $select_plate_id_fd->mold_id_fd }}</option>
                                        @endforeach
    ...
    <div class="container-fluid">
        <div class="panel panel-info">
            @include('frontend.printing.includes.nav', ['mainTitle' => 'Production Department / Plate Maintenance'])
            <br>
            <div class="panel-body">
                ...
                <div class="row">
                    ...
                    <div class="col-lg-4">
                        <div class="input-group">
                            <span class="input-group-addon input-group-addon-custom" id="sizing-addon2">Part No</span>
    ...
    <script>
        ...
        $('#ctp-table').on( 'click', 'tr', function () {
            if ( $(this).hasClass('selected') ) {
                ...
                selectedPrinter = table.row( this ).data().printer;
                selectedFlexiDie = table.row( this ).data().mold_id_fd;
...
/*
    ...
    <script>
                    ...
                    Yes: function(yesButton){
                         ...
                         $.ajax({
                            ...
                            data: { 
                                ...
                                printer : $('#select_printer').val(),
                                plate_size : $('#select_plate_size').val(),
                                mold_id_fd : $('#select_mold_id_fd').val(),
                                location : $('#text_location').val(),
                                print_req : $('#select_print_req').val(),
                                qty : $('#input_qty').val(),
                                qty_life : $('#input_qty_life').val(),
        ...
        $('#select_req_id').change(function(){           
            ...
            $('#select_mac_no').LoadingOverlay("show");
            $('#select_print_req').LoadingOverlay("show");
            $('#input_qty').LoadingOverlay("show");
            $('#text_film_desc').LoadingOverlay("show");
            $('#input_qty_film').LoadingOverlay("show");
            $('#text_remark').LoadingOverlay("show");
            $("#select_part_no").prop('disabled',true);
            $("#select_cust_no").prop('disabled',true);
            $("#select_mac_no").prop('disabled',true);
            $("#select_print_req").prop('disabled',true);
            $("#input_qty").prop('disabled',true);
            $("#text_film_desc").prop('disabled',true);
            $("#select_custinput_qty_film_no").prop('disabled',true);
            $("#text_remark").prop('disabled',true);
            $.get('/ctp/selectDataReq', {req_id: $('#select_req_id').val()},  function(data) {
                ...
                $('#select_mac_no').val(data.mac_no);
                $('#select_print_req').val(data.print_req);
                $('#input_qty').val(data.qty);
                $('#text_film_desc').val(data.film_desc);
                $('#input_qty_film').val(data.qty_film);
                $('#text_remark').val(data.remark);
                $('.selectpicker').selectpicker('refresh');
                $('#select_part_no').LoadingOverlay("hide");
                $('#select_cust_no').LoadingOverlay("hide");
                $('#select_mac_no').LoadingOverlay("hide");
                $('#select_print_req').LoadingOverlay("hide");
                $('#input_qty').LoadingOverlay("hide");
                $('#text_film_desc').LoadingOverlay("hide");
                $('#input_qty_film').LoadingOverlay("hide");
                $('#text_remark').LoadingOverlay("hide");
                $("#select_part_no").prop('disabled',false);
                $("#select_cust_no").prop('disabled',false);
                $("#select_mac_no").prop('disabled',false);
                $("#select_print_req").prop('disabled',false);
                $("#input_qty").prop('disabled',false);
                $("#text_film_desc").prop('disabled',false);
                $("#select_custinput_qty_film_no").prop('disabled',false);
                $("#text_remark").prop('disabled',false);
                $('.selectpicker').selectpicker('refresh');
            }); 
        });
    ...
    <script>
        ...
        $(function(){
            $('#btnPlateModify').click(function(e){
                ...
                $("#select_printer_modify").val(selectedPrinter);
                $("#select_plate_size_modify").val(selectedMoldSize);
                $("#select_mold_id_fd_modify").val(selectedFlexiDie);
                    ...
                    Yes: function(yesButton){
                         ...
                         $.ajax({
                            ...
                            data: { 
                                ...
                                printer : $('#select_printer_modify').val(),
                                plate_size : $('#select_plate_size_modify').val(),
                                mold_id_fd : $('#select_mold_id_fd_modify').val(),
                                location : $('#text_location_modify').val(),
                                print_req : $('#select_print_req_modify').val(),
                                //qty : $('input[id="input_qty_modify"]').val(),
                                qty_life : $('#input_qty_life_modify').val(),
