...
@include('frontend.ctp.includes.nav')
  <link href="http://cdnjs.cloudflare.com/ajax/libs/summernote/0.8.8/summernote.css" rel="stylesheet">
      <h3 class="box-title"> {!! trans('REPEAT CONFIRMATION ORDER') !!} <small>(For CTP department)</small> </h3>
  <div class="row" id="app">
    <div class="col-lg-12">
      <p class="text-right">No:   {!!$repeat->rsc_number!!}  </p>
    </div>
    <div class="col-lg-12">
            {!! Form::model($repeat, array('route' => array('frontend.ctp.repeatUpdate', $repeat->id), 'method' => 'PUT')) !!}
            ...
            <div class="form-group row">
              ...
              <div class="col-md-10">{!! Form::textarea('remark', $repeat->remark, array('class' => 'form-control', 'id' => 'summernote')) !!}</div>
            </div>
              <div class="form-group row">
                {!! Form::label('remark2', 'Remarks (for Planning department)', ['class' => 'col-md-2']) !!}
                <div class="col-md-10">{!! Form::textarea('remark2', $repeat->remark2, array('class' => 'form-control', 'id' => 'summernote2')) !!}</div>
              </div>
              @permission('ctp')
              <div class="form-group row {{ $errors->has('remark3') ? 'has-error' : '' }}">
                {!! Form::label('remark3', 'Remarks (for CTP department)', ['class' => 'col-md-2']) !!}
                <div class="col-md-10">{!! Form::textarea('remark3', '', array('class' => 'form-control', 'id'=>'summernote3')) !!}</div>
                <span class="text-danger">{{ $errors->first('remark3') }}</span>
              </div>
